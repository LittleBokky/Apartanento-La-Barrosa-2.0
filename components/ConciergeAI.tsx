...
export const ConciergeAI: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const labels = {
    es: { title: 'Asistente La Barrosa', placeholder: 'Pregúntame algo...', send: 'Enviar' },
    en: { title: 'La Barrosa Assistant', placeholder: 'Ask me anything...', send: 'Send' }
  };
        ...
        <div className="w-[350px] max-w-[calc(100vw-2rem)] h-[500px] bg-white dark:bg-surface-dark shadow-2xl rounded-2xl border border-gray-100 dark:border-gray-800 flex flex-col overflow-hidden mb-4 animate-fade-in-up">
          <div className="p-4 bg-primary text-white flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold">{labels[lang].title}</span>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 p-4 overflow-y-auto space-y-4">
            {messages.length === 0 && (
              <div className="text-center text-text-muted dark:text-gray-400 py-10">
                ...
                <p className="text-sm">
                  {lang === 'es' 
                    ? '¡Hola! Soy tu conserje virtual. ¿En qué puedo ayudarte hoy?' 
                    : 'Hi! I am your virtual concierge. How can I help you today?'}
                </p>
              </div>
            )}
            {messages.map((msg, i) => (
